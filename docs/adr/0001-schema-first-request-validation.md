...
publishes today carries no constraints at all — so the TypeScript frontend re-implements every
...
removes — hand-maintaining the same rules in two languages — is per-endpoint and compounds.
...
- **FluentValidation.** Better ergonomics than attributes for conditional rules, but the rules become
  invisible to OpenAPI entirely — strictly worse for the thing we are trying to fix.
...
- Edge validation covers shape, length and format only. Aggregates keep their own guards — they are
...
- Rules that depend on the clock — "date of birth is not in the future" — cannot go in a static
